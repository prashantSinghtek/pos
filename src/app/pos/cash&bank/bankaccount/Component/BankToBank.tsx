...
export default function BankToBank({ data , SetBankToBankTransfer }: any) {
    ...
    const handleChangedbankTo = (selectedOption: any) => {
        console.log("selected csssssswwwwwss--->>>", selectedOption);
        setSelectedbankto(selectedOption.id);
            ...
            <Formik
                ...
                onSubmit={async (values, { setSubmitting }) => {
                    try {
                        ...
                        const response = await auth.AddBankToBank(token,Selectedbank,Selectedbankto, formData);
                ...
                {({
                    ...
                    <>
                        <div className="flex flex-wrap gap-5 my-5 w-full">
                            ...
                            <div className="w-[45%]">
                                <div className="w-[100%] flex flex-col space-y-2 ">
                                    <div className="text-[#808080]">To</div>
                                    <Select
                                        name="Accountdisplayname"
                                        options={allbank}
                                        value={Selectedbank?.value}
                                        onChange={handleChangedbankTo}
                                        styles={customStyles}
                                        className="w-full  bg-white  rounded-md outline-none font-medium font-optima  text-primary text-sm focus-within:outline-gray-200 focus-within:outline focus-within:outline-2"
                                    />
                                </div>
