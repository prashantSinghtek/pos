...
export default function AddCashSale({ product ,defaultSelectedproduct}: any) {
  ...
  const defaultParty = parties?.find((item: any) => item.id == defaultSelectedproduct?.partiesName);
  console.log(defaultParty);
  
  const [selectedParties, setSelectedParties] = useState<any>({
    value: defaultParty?.partyName?.toUpperCase() || '',
    label: defaultParty?.partyName?.toUpperCase() || '',
    id: defaultSelectedproduct?.partiesName || '',
  });
  
  // Log state to verify
  console.log(selectedParties);
  
  // Ensure to update state if needed
  useEffect(() => {
    if (defaultParty) {
      setSelectedParties({
        value: defaultParty.partyName?.toUpperCase(),
  ...
  console.log("Selectedproduct", defaultSelectedproduct)
  return (
    <>
      <Formik
        initialValues={{
          partiesName: defaultSelectedproduct?.partiesName || "",
          phonenumber: defaultSelectedproduct?.phoneNumber || "",
          invoicenumber: defaultSelectedproduct?.invoiceNumber || "",
          invoicedate: defaultSelectedproduct?.invoiceDate || new Date().toISOString().split('T')[0],
          paymenttype: defaultSelectedproduct?.partiesName || "",
          roundoff: defaultSelectedproduct?.roundOff ||  "",
          total: defaultSelectedproduct?.totalInvoiceAmount || "",
          advanceamount: defaultSelectedproduct?.receivedAmount|| "",
          description: defaultSelectedproduct?.description || " ",
          RefNumber: defaultSelectedproduct?.referenceNumber ||  "",
          BillingAddress: defaultSelectedproduct?.billingAddress || "",
        }}
        validationSchema={""}
